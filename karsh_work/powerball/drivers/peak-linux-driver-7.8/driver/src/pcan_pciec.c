...
static int pita_getscl(void *data)
{
  ...
  pita_set_scl_highz(card); // set tristate
...
static int pita_getsda(void *data)
{
  ...
  pita_set_sda_highz(card); // set tristate
...
static int initPCA9553(PCAN_PCIEC_CARD *card)
{
  ...
  error = writeBytePCA9553(card, 1, 44 / 1);  // prescaler for frequency 0: "SLOW" = 1 Hz = "44"
  if (error)
    goto fail;
  error = writeBytePCA9553(card, 2, 0x80) ;   // duty cycle 0: 50%
  if (error)
    goto fail;
  error = writeBytePCA9553(card, 3, 44 / 5) ; // prescaler for frequency 1: "FAST" = 5 Hz
  if (error)
    goto fail;
  error = writeBytePCA9553(card, 4, 0x80) ;   // duty cycle 1: 50%
  if (error)
    goto fail;
  card->PCA9553_LS0Shadow = PCA9553_LS0_INITIALIZED;
  error = writeBytePCA9553(card, 5, card->PCA9553_LS0Shadow) ; // switch LEDs and ... to initial state
  if (error)
    goto fail;
...
static void deinitPCA9553(PCAN_PCIEC_CARD *card)
{
  ...
  writeBytePCA9553(card, 5, card->PCA9553_LS0Shadow) ; // switch LEDs and ... to off
}
...
/**
 * Initialize the I2C subsystem for one or more CAN channels. Only call for blinking PCIEC masters.
 * Please note that this function skips hardware initialization for other channels if there is a master channel
 * for multiple CAN channels sharing I2C hardware.
 * @param card The associated card structure
 * @return The error code
 */
static int pcan_initI2C(struct pci_dev *pciDev, PCAN_PCIEC_CARD *card, void *pvVirtConfigPort)
{
  int error = 0;
  ...
  // create the card's special i2c_adapter structure with reference to the bit banging interface
...
/**
 * Set VCC to external circuit on or off. Please note, setting is done in background.
...
/**
 * Deinitializes and frees any associated I2C subsystem from the CAN channel. only call for blinking PCIEC masters.
 * @param card The associated PCAN-ExpressCard
 */
static void pcan_deinitI2C(PCAN_PCIEC_CARD *card)
{
  ...
  initPITAGPIO(card); // init equals deinit
...
static void pciec_activity_scanner(struct work_struct *work)
{
  ...
  for (i = 0; i < PCIEC_CHANNELS; i++)
  {
    ...
    if (dev != NULL)
    {
      ...
      u8 offset = (dev->port.pci.nChannel) ? PCA9553_LED1_OFFSET : PCA9553_LED0_OFFSET;
      ...
      switch(state)
      {
        case ACTIVITY_XMIT:
          ...
          dev->ucActivityState = ACTIVITY_IDLE;  // automatic fallback
          break;
...
PCAN_PCIEC_CARD *pcan_pciec_create_card(struct pci_dev *pciDev, struct pcandev *dev)
{
  ...
  if ((card = (PCAN_PCIEC_CARD *)kmalloc(sizeof(PCAN_PCIEC_CARD), GFP_KERNEL)) == NULL)
  {
    // TODO error check
    return NULL;
  }
  ...
  card->dev[0] = dev;
  pcan_initI2C(pciDev, card, dev->port.pci.pvVirtConfigPort);      // TODO error check
...
PCAN_PCIEC_CARD *pcan_pciec_locate_card(struct pci_dev *pciDev, struct pcandev *dev)
{
  ...
  // it is a bit complicated to get the master channel association since the device did not know its sibling
  for (ptr = pcan_drv.devices.prev; ptr != &pcan_drv.devices; ptr = ptr->prev)
  {
    ...
    if ((local_dev != NULL) &&
        ...
        (local_dev->port.pci.dwConfigPort == dev->port.pci.dwConfigPort))
    {
...
void pcan_pciec_delete_card(struct pcandev *dev)
{
  ...
  if (dev->port.pci.card != NULL)
  {
    ...
    else
    {
      ...
      card->dev[dev->port.pci.nChannel] = NULL;  // remove card's association to device
    }

    dev->port.pci.card = NULL;  // remove device's association to card
  }
}
